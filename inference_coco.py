...
def inference_coco(model_path, coco_path, image_dir, output_dir, output_csv):
    with tf.Session(graph=tf.Graph()) as sess:
            ...
            if img is not None:
                try:
                    ...
                    score, classes, boxes, nums_det, keypoint, keypoint_score = sess.run(
                        [det_score, det_class, det_boxes,
                         det_numbs, det_keypoint,
                         det_keypoint_score],
                        feed_dict={
                            input_tensor: image_exp_dims})
        ...
        if len(error_image_ids) > 0:
            raise ValueError(f"Error in image ids: {error_image_ids}")
...
if __name__ == '__main__':
    ...
    inference_coco(args.model_path, args.coco_json_path, args.image_dir, args.output_dir, args.output_csv)
